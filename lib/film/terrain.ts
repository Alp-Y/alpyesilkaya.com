...
export type Terrain = {
  ...
  contours: number[][];
  /** display mesh for the dig animation: both surveys in one TIN, both levels at every vertex */
  mesh: { x: number; y: number; eg: number; ex: number }[];
  meshTris: [number, number, number][];
...
/** basin base: x 34…86, y 22…48, level 606.20 falling 0.2 % to the east; batters 1:2 (components/film/plan.ts draws the same outline) */
export const BASE = { x0: 34, x1: 86, y0: 22, y1: 48 };
...
export function buildTerrain(low = false): Terrain {
  ...
  // one mesh over the whole window so the existing ground can sink into the basin without seams
  const all: P3[] = [...egPts, ...exPts.filter((p) => !egPts.some((q) => Math.abs(q.x - p.x) < 0.05 && Math.abs(q.y - p.y) < 0.05))];
  const meshTris = delaunay(all.map((p) => [p.x, p.y] as XY));
  const mesh = all.map((p) => {
    const g = eg.at(p.x, p.y) ?? p.z;
    return { x: p.x, y: p.y, eg: g, ex: Math.min(g, ex.at(p.x, p.y) ?? g) };
  });
