...
export function reportModel(r: EngineResult, date = new Date()): ReportModel {
  ...
  return {
    ...
    zones: c.zones.map((z) => ({ id: z.id, range: `CH ${chainage(z.from)} – ${chainage(z.to)}`, area: z.area, cut: z.cut, share: z.cut / total })),
    bands: c.bands.map((b) => ({ label: b.to === null ? `> ${b.from.toFixed(1)} m` : `${b.from.toFixed(1)} – ${b.to.toFixed(1)} m`, area: b.area, cut: b.cut, share: b.cut / total })),
    critical,
    sections: stations,
