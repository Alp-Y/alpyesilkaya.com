...
export const env = (t: number, a: number, b: number, c = Infinity, d = Infinity) => Math.min(easeOut(seg(t, a, b)), c === Infinity ? 1 : 1 - easeInOut(seg(t, c, d)));
...
/** `ox`, `oy` move the camera's centre on screen (px), to make room for a panel. */
export function view(f: Frame, cam: Cam, ox = 0, oy = 0): View {
...
/** Two-line section caption, e.g. From coordinates / to surfaces. Shrinks to fit `maxW`. */
export function caption(f: Frame, lines: [string, string], a: number, where: "tl" | "bl" | "center" = "tl", maxW?: number) {
  if (a <= 0) return;
  const pad = f.tall ? 18 : Math.max(24, f.W * 0.028);
  let size = f.tall ? Math.min(26, f.W * 0.066) : clamp(f.W * 0.028, 20, 36);
  const room = maxW ?? f.W - pad * 2;
  const widest = Math.max(textWidth(f, lines[0], size, "sans", 560), textWidth(f, lines[1], size, "sans", 560));
  if (widest > room) size *= room / widest;
  const lh = size * 1.04;
  const rise = (1 - easeOut(a)) * 10;
  let x = pad;
  let y = pad + size + (f.tall ? 4 : 8);
...
export function marker(f: Frame, text: string, a: number) {
  // phones: the caption needs the whole width
  if (a <= 0 || f.tall) return;
  const pad = f.tall ? 18 : Math.max(24, f.W * 0.028);
...
export function table(f: Frame, cols: Col[], rows: string[][], o: TableOpts) {
  ...
  rows.forEach((r, ri) => {
    ...
    r.forEach((v, ci) => {
      ...
      // text that would run into the next column is shortened with an ellipsis
      let text = v;
      const room = cell(ci) - padX * 1.4;
      if (!isNum && textWidth(f, text, size) > room) {
        while (text.length > 1 && textWidth(f, `${text}…`, size) > room) text = text.slice(0, -1);
        text = `${text.trimEnd()}…`;
      }
      (isNum ? num : mono)(f, text, tx + dx, y + o.rowH * 0.64, { size, color, align: isNum ? "right" : "left", spacing: isNum ? 0 : size * 0.05 });
    });
...
/** A table panel only as tall as its rows (plus the footer line), centred in the room it has. */
export function fitPanel(f: Frame, r: { x: number; y: number; w: number; h: number }, rows: number, scale: number, rowFactor: number) {
  const size = monoSize(f) * scale;
  const h = Math.min(r.h, size * 3.4 + size * rowFactor * (rows + 1) + size * 4);
  return { ...r, y: f.tall ? r.y : r.y + (r.h - h) / 2, h };
}
